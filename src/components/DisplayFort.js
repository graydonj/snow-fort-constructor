...
function DisplayFort ({fights, fort, removeFortItem, baseFort, repairFortItem}) {
    ...
    <>
      ...
      <div className="my-fort">
        <div className="fort-section"><h4>WALLS</h4>
          {fort.map((fortItem) =>
            (fortItem.name === "wall") ? (
              <DisplayFortSection key={fortItem.id} fortItem={fortItem} removeFortItem={removeFortItem} baseHealth={baseFort[0].health} repairFortItem={repairFortItem}/>
              ) : null )}
        </div>
        <div className="fort-section"><h4>FORTIFICATIONS</h4>
          {fort.map((fortItem) =>
            (fortItem.name === "fortification") ? (
              <DisplayFortSection key={fortItem.id} fortItem={fortItem} removeFortItem={removeFortItem} baseHealth={baseFort[2].health} repairFortItem={repairFortItem}/>
            ) : null)}
        </div>
        <div className="fort-section"><h4>TOWERS</h4>
          {fort.map((fortItem) =>
            (fortItem.name === "tower") ? (
              <DisplayFortSection key={fortItem.id} fortItem={fortItem} removeFortItem={removeFortItem} baseHealth={baseFort[1].health} repairFortItem={repairFortItem}/>
            ) : null)}
        </div>
        <div className="fort-section"><h4>KEEPS</h4>
          {fort.map((fortItem) =>
            (fortItem.name === "keep") ? (
              <DisplayFortSection key={fortItem.id} fortItem={fortItem} removeFortItem={removeFortItem} baseHealth={baseFort[3].health} repairFortItem={repairFortItem}/>
